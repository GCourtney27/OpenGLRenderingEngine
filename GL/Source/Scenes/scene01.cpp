...
bool Scene01::Initialize()
{
	// Shader
	m_shader = new Shader();
	m_shader->CompileShader(m_engine->Get<FileSystem>()->GetPathname() + "Shaders\\Vertex Shaders\\basic.vs", GL_VERTEX_SHADER);
	m_shader->CompileShader(m_engine->Get<FileSystem>()->GetPathname() + "Shaders\\Fragment Shaders\\basic.fs", GL_FRAGMENT_SHADER);
	m_shader->Link();
	m_shader->Use();
