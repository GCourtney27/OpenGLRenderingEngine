...
void main()
{
	...
	for (int i = 0; i < 5; i++)
	{
		normal = texture(normalSample, fragment_uv).rgb;
		normal = normalize((normal * 2.0) - vec3(1.0));
		phong(i, fragment_position, normal, diffuse, specular);
		final_color += (diffuse + specular);
	}

	color = texture(textureSample, fragment_uv) * vec4(final_color + diffuse, 1.0) + vec4(specular, 1.0);

}
//this shader is a copy of phong_multi.fs